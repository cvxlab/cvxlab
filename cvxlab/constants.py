...
class Constants:
    ...
    class ConfigFiles:
        ...
        INPUT_DATA_FILE: Default name for the input data file (only in case 
            of single input data file).
        ...
        INSTANCES_DIR: Default name for the directory containing Model instances 
            saved by the user.
        """
    ...
    class DefaultStructures:
        ...
        SET_STRUCTURE: 
        Structure for defining sets with metadata and filters. Sets are defined 
        as the dimensions of the numerical problem. 

        Properties of a SET:
        - set_key: name of the set, used as name of the set in the SQLite data 
        ...
        - description: (Optional) information of the set provided by the modeler.
        - split_problem: (Optional) if the set items defines independent numerical 
        ...
        - copy_from: (Optional) key of another set to copy the data from. If a
            set is defined with this key, it will copy the data from the set with
            the key defined in this field, and it is not necessary to provide the
            related data in the sets file.
        - filters: (Optional) dictionary with keys as the filters name and values
            as the list of filter values. Filters are used to define variables 
            as part of data tables.
        - aggregations: (Optional) list of set aggregations useful to aggregate 
            data for visualization. Not recalled in operating with numerical problem.


        DATA_TABLE_STRUCTURE: 
        Structure for defining data tables with variables and their properties.
        Data tables correspond to SQLite tables and are used to generate variables
        in the numerical problem. 

        Properties of a DATA TABLE:
        - table_key: name of the data table, used as name of the table in the 
        ...
        - variables_info: definition of the variables based on the same data table.
            It is a dictionary with keys as variables names and values as dict with
            variable info. Variables are case sensitive.
            - value: (Optional) ALLOWED_CONSTANTS (only for constants!).
            - blank_fill: (Optional) numerical value that will be used to fill variables
        ...
        PROBLEM_STRUCTURE: 
        Structure for defining problems with objectives and expressions.

        Properties of a PROBLEM:
        - problem_key: (Optional) name of the problem, used to define multiple problems
            in the same model. If not defined, the problem is considered as the main problem.
        - objective: (Optional) list of expressions defining the objective of the problem.
        ...
        - expressions: list of expressions defining the problem constraints. Each expression
        ...
        - description: (Optional) list of strings with the problem description provided
            by the modeler. It is used to provide additional information about the problem.

        XLSX_PIVOT_KEYS and XLSX_TEMPLATE_COLUMNS: 
        Keys and columns used to pivot data for generating Excel template files 
        for sets, variables, and problems.

        ALLOWED_BOOL: 
        Dictionary mapping common boolean representations to Python boolean values.
        This is used to validate boolean values in the data files.
        """
    ...
    class SymbolicDefinitions:
        ...
        - TOKEN_PATTERNS: Dictionary of regex patterns for token types (text, 
        ...
        - USER_DEFINED_CONSTANTS: Dictionary of user-defined constants. These are
            ...
            field in data tables). Constants are be built with exogenous data only. 
        - USER_DEFINED_OPERATORS: Dictionary of user-defined operators. Keys of
            operators can be directly used in symbolic expressions. These are
            defined within the dictionary values (complex operators are defined
            in util_operators module). Operators are built with exogenous data only. 
        """
    ...
    class NumericalSettings:
        ...
        - STD_VALUES_TYPE: Type of standard numerical values.
        - ALLOWED_VALUES_TYPES: Tuple of allowed numerical types for values. Useful
            to recognize and homogenize numerical values provided by user in input 
            data files.
        - ALLOWED_TEXT_TYPE: Type of allowed text values.
        - ALLOWED_SOLVERS: List of installed solvers available for use. Same solvers
            as in cvxpy.installed_solvers().
        - DEFAULT_SOLVER: Default solver to be used if not specified by the user.
        - TOLERANCE_TESTS_RESULTS_CHECK: Tolerance for checking results of tests. 
            It is a relative difference (0.02 means 2% of maximum allowed difference
            between the resulting database and the test databases).
        - TOLERANCE_MODEL_COUPLING_CONVERGENCE: Tolerance for convergence of model
            coupling. It is a relative difference (0.01 means 1% of maximum allowed
            difference between the resulting database and the previous iteration 
            databases, considering the highest difference between the values of 
            all the tables).
        - MAXIMUM_ITERATIONS_MODEL_COUPLING: Maximum number of iterations for model
            coupling. It is used to limit the number of iterations in case of
            convergence issues.
        - ROUNDING_DIGITS_RELATIVE_DIFFERENCE_DB: Number of digits to round the
            relative difference between the values of the database and the test
            database. It is used to avoid numerical issues in case of very small
            differences between the values of the database and the test database.
        - SPARSE_MATRIX_ZEROS_THRESHOLD: Threshold for considering a matrix as sparse.
            (0.3 means 30% of zeros in the matrix). If the percentage of zeros 
            in the matrix is higher than this threshold, the matrix is considered 
            as sparse and stored in a sparse format to save memory.
        - SQL_BATCH_SIZE: Batch size for SQL operations. It is used to limit the
            number of rows processed in a single SQL operation to avoid memory 
            issues and speed up database operations.
        """
